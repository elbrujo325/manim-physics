"""
Campo Eléctrico - @lomejorphysics (Reel Edition) - CORREGIDO
================================================
TAMAÑOS AUMENTADOS para Instagram Reels (legible en móvil)

Compilar con:
/home/odoo-01/bin/micromamba run -n manim manim -p -qh --fps 60 -r 1080,1920 campo_electrico_big.py CampoElectrico
"""
...
# ═══════════════════════════════════════════
# ESCALAS PARA REELS (AUMENTADAS PARA MÓVIL)
# ═══════════════════════════════════════════
SCALE_TITLE = 240      # Títulos principales (antes 180)
SCALE_EQ = 60          # Ecuaciones principales (antes 44)
SCALE_SUBEQ = 48       # Sub-ecuaciones (antes 30-38)
SCALE_TEXT = 36        # Texto descriptivo (antes 18-24)
SCALE_LABEL = 44       # Etiquetas de vectores (antes 28-32)
...
class CampoElectrico(MovingCameraScene):
    ...
    # ─────────────────────────────────────────
    def intro_reel(self):
        titulo = MathTex("\\vec{E}", font_size=SCALE_TITLE).set_color_by_gradient(BLUE, TEAL)
        sub = Text("Campo Eléctrico", font_size=SCALE_TEXT + 16, color=GRAY_A)
    ...
    def geometria_vectorial(self):
        header = Text("I. Geometría del problema", font_size=SCALE_TEXT + 4, color=GRAY_B)
        ...
        axes = Axes(
            ...
            x_length=9,
            y_length=7,
            axis_config={"color": GRAY, "stroke_width": 1.5, "include_ticks": False},
        ).shift(DOWN*0.3)
        ...
        origin_label = MathTex("O", font_size=SCALE_LABEL, color=WHITE).next_to(origin_dot, DL, buff=0.15)
        ...
        label_Q = MathTex("Q", font_size=SCALE_LABEL, color=COLOR_SOURCE).next_to(charge_Q, UL, buff=0.15)
        label_q = MathTex("q", font_size=SCALE_LABEL, color=COLOR_TEST).next_to(charge_q, DR, buff=0.15)
        ...
        vec_rp_label = MathTex("\\mathbf{r}'", font_size=SCALE_LABEL, color=COLOR_VEC_RP)
        ...
        vec_r_label = MathTex("\\mathbf{r}", font_size=SCALE_LABEL, color=COLOR_VEC_R)
        ...
        vec_sep_label = MathTex(
            "\\boldsymbol{r}", font_size=SCALE_LABEL + 4, color=COLOR_SEP
        ...
        self.geo_group = VGroup(
            ...
            vec_rp, vec_rp_label, vec_r, vec_r_label, vec_sep, vec_sep_label,
            sep_eq, header,
        )
    ...
    def ley_coulomb_formal(self):
        header = Text("II. Ley de Coulomb", font_size=SCALE_TEXT + 4, color=GRAY_B)
        header.to_edge(UP, buff=0.6)
        self.play(FadeIn(header))
        ...
        sep_reminder = MathTex(
            "\\boldsymbol{r} = \\mathbf{r} - \\mathbf{r}'"
            "\\,,\\quad r = |\\boldsymbol{r}|",
            font_size=SCALE_SUBEQ, color=GRAY_A,
        ...
        nota_r2 = Text("Decae con el cuadrado de la distancia", font_size=SCALE_TEXT, color=TEAL, slant=ITALIC)
        ...
        superpos = MathTex(
            "\\vec{F}_{\\text{total}}", "=", "\\frac{1}{4\\pi\\varepsilon_0}",
            "\\sum_{i=1}^{n}", "\\frac{q\\, q_i}{r_i^{\\,2}}", "\\hat{r}_i",
            font_size=SCALE_SUBEQ,
        ).next_to(sep_reminder, DOWN, buff=1.2)
        
        nota_sup = Text("Principio de superposición lineal", font_size=SCALE_TEXT, color=GREEN_A, slant=ITALIC)
    ...
    def definicion_campo_electrico(self):
        header = Text("III. Campo eléctrico", font_size=SCALE_TEXT + 4, color=GRAY_B)
        header.to_edge(UP, buff=0.6)
        self.play(FadeIn(header))

        # Paso 1: F / q₀
        step1 = MathTex(
            "\\vec{E}", "\\equiv", "\\lim_{\\Delta q \\to 0}",
            "\\frac{\\vec{F}_{\\Delta q}}{\\Delta q}",
            font_size=SCALE_EQ + 4,
        ).shift(UP*1.5)
        step1[0].set_color(BLUE)
        
        nota = Text(
            "La carga de prueba no debe perturbar la fuente",
            font_size=SCALE_TEXT, color=BLUE_A, slant=ITALIC,
        ...
        step2 = MathTex(
            "\\vec{E}(\\mathbf{r})", "=", "\\frac{1}{4\\pi\\varepsilon_0}",
            "\\frac{Q}{r^2}", "\\hat{r}",
            font_size=SCALE_EQ + 2,
    ...
    def visualizacion_campo(self):
        header = Text("IV. Visualización", font_size=SCALE_TEXT + 4, color=GRAY_B)
        header.to_edge(UP, buff=0.6)
        self.play(FadeIn(header))

        # Carga fuente
        source = self.get_charge(1, pos=ORIGIN, color=COLOR_SOURCE)
        label_q = MathTex("+Q", font_size=SCALE_LABEL).next_to(source, DR, buff=0.1)
        ...
        test_lbl = MathTex("q_0", color=COLOR_TEST, font_size=SCALE_LABEL).next_to(test, DOWN)
        ...
        f_eq = MathTex(
            "\\vec{F} = q_0\\,\\vec{E}",
            color=YELLOW, font_size=SCALE_SUBEQ,
        ).to_edge(DOWN, buff=1)
        ...
        self.play(FadeOut(
            stream, source, label_q, test, test_lbl, force_arrow, f_eq, header,
        ))
    ...
    def cierre(self):
        msg = Text(
            ...
            font_size=SCALE_TEXT + 4, color=BLUE_A,
            line_spacing=1.4,
        ).move_to(ORIGIN)
        
        canal = Text("@lomejorphysics", font_size=SCALE_TEXT + 2, color=GRAY_B)
